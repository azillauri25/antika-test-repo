...
if (!empty($search)) {
    ...
    $searchQuery = "WHERE c.nama_cust LIKE '%$search%' OR c.username LIKE '%$search%'";
}
// Ambil data pelanggan dengan jumlah transaksi
$query = "SELECT c.customer_ID, c.nama_cust, c.username, c.email_cust, c.telepon_cust, c.alamat_cust, COUNT(o.order_ID) AS jumlah_transaksi
    FROM customer c
...
<div class="header">
    <div class="username">
    Selamat Datang, Admin <?php echo htmlspecialchars($nama_karyawan); ?>
    </div>
...
<div class="content content-main">
    ...
    <!-- <div class="mb-3 float-left">
        <a href="?sort=customer" class="btn btn-primary" style="font-size:10px;">Urutkan Berdasarkan Customer ID</a>
        <a href="?sort=transaksi" class="btn btn-light" style="font-size:10px;">Urutkan Berdasarkan Jumlah Transaksi</a>
    </div>
        ...
        <tbody>
            ...
            if ($result_pelanggan && mysqli_num_rows($result_pelanggan) > 0) {
                while ($row = mysqli_fetch_assoc($result_pelanggan)) {
                    ...
                    echo "<td>" . htmlspecialchars($row['nama_cust']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['username']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['email_cust']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['telepon_cust']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['alamat_cust']) . "</td>";
                    echo "<td>" . htmlspecialchars($row['jumlah_transaksi']) . "</td>";
                    echo "<td><a href='edit_pelanggan.php?customer_ID=" . htmlspecialchars($row['customer_ID']) . "' class='btn btn-warning btn-sm' style='font-size:10px;''>Edit</a></td>";
...
function konfirmasiHapus(customer_ID) {
    ...
    var input = prompt("Ketik 'hapus' untuk menghapus pelanggan.");

    // Cek apakah input sama dengan "hapus"
    if (input === "hapus") {
        // Jika sesuai, lanjutkan ke halaman hapus

            ...
            if ($result_pesanan && mysqli_num_rows($result_pesanan) > 0) {
                while ($row = mysqli_fetch_assoc($result_pesanan)) {
                    if ($currentOrderID != $row['order_ID']) {
                        ...
                        if ($row['status_pesanan'] == 'menunggu validasi pembayaran') {
                            echo "<td rowspan='{$produkCount}'>
                                <form method='POST' action=''>
                                    ...
                                    <button type='submit' class='btn btn-success admin-laporan' style='display: block; margin: 0 auto;'>Validasi</button>
                                </form>
                                <form method='POST' action='' style='margin-top: 5px;'>
                                    <input type='hidden' name='order_ID' value='" . htmlspecialchars($row['order_ID']) . "'>
                                    <input type='hidden' name='status_pesanan' value='pesanan ditolak'>
                                    <button type='submit' class='btn btn-danger admin-laporan' style='display: block; margin: 5px auto;'>Tolak</button>
                                </form>
                            </td>";
                        } elseif ($row['status_pesanan'] == 'pesanan diterima') {
                            // Jika pesanan sudah diterima
                            echo "<td rowspan='{$produkCount}' style='text-align: center;'>Pesanan Diterima</td>";
                        } elseif ($row['status_pesanan'] == 'pesanan ditolak') {
                            // Jika pesanan ditolak
                            echo "<td rowspan='{$produkCount}' style='text-align: center;'>Pesanan Ditolak</td>";
                        } else {
                        ...
                        $totalHarga = $row['kuantitas'] * $row['harga'];
                        echo "<tr>";
                        echo "<td>" . htmlspecialchars($row['nama_produk']) . "</td>";
                        echo "</tr>";
                    }

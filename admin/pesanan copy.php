        ...
        <?php
            ...
            if ($result_pesanan && mysqli_num_rows($result_pesanan) > 0) {
                while ($row = mysqli_fetch_assoc($result_pesanan)) {
                    ...
                    // Cek apakah waktu pengiriman dan waktu sampai ada
                    if ($row['status_pesanan'] === 'pesanan selesai') {
                        // Cek waktu pengiriman dan waktu sampai
                        $waktuKirim = !empty($row['waktu_pengiriman']) ? DateTime::createFromFormat('Y-m-d H:i:s', $row['waktu_pengiriman']) : null;
                        $estimasiSampai = !empty($row['waktu_sampai']) ? DateTime::createFromFormat('Y-m-d H:i:s', $row['waktu_sampai']) : null;
                    
                        // Cek apakah kedua waktu ada
                        if ($waktuKirim && $estimasiSampai) {
                            // Jika ada, tampilkan waktu
                            echo "<div style='text-align: left;'>";
                            echo "Waktu Kirim:<br>"; 
                            echo "<span style='font-weight: bold;'>" . htmlspecialchars($waktuKirim->format('d-m-Y H:i')) . "</span><br>"; // Tanggal
                            echo "</div>";
                            echo "<div style='text-align: left;'>";
                            echo "Estimasi Sampai:<br>"; 
                            echo "<span style='font-weight: bold;'>" . htmlspecialchars($estimasiSampai->format('d-m-Y H:i')) . "</span><br>"; // Tanggal
                            echo "</div>";
                        } else {
                            // Jika salah satu waktu tidak ada, tampilkan pesan "pesanan tidak valid"
                            echo "<span><strong>Pesanan tidak valid</strong></span>";
                        }
                    } else if ($row['status_pesanan'] === 'pesanan dikirim') {
                        // Logika untuk status pesanan 'dikirim'
